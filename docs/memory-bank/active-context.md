...
- 最终安全候选 `69b63723e31ca3365fe2e52695229509e22b5999` 使用 Node.js `24.18.0`、pnpm `10.33.2` 和 PostgreSQL 服务端 `17.2` 完成复验。独立 `pnpm verify` 为 66 个文件通过、10 个文件跳过，853 项通过、98 项跳过，退出码 0。
- 独立 `pnpm verify` 分包结果为 protocol 97/0、core 183/0、adapters 260/40、companion 10/0、knowledge-agent 26/45、CLI 29/0、knowledge-server 248/13，数字顺序均为通过数/跳过数。
- PostgreSQL `17.2` 上的 `pnpm verify:cloud` 退出码 0：内部全门禁 948 项通过/3 项跳过，PostgreSQL 阶段 368/3，A2A 阶段 261/0；按脚本实际执行次数合计 1,577 项通过、6 项跳过，重复套件保留分阶段口径。
