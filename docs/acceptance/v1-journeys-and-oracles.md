...
- **Oracle:** all four real Connector rows are simultaneously visible and truthful in the same Sources view; one unique approved leaf from each Connector enters the same selected-leaf manifest and active QMD generation; every fact traverses the real provider public surface to that generation and a resolvable citation; Feishu records and explicitly invokes the approved profile, redacted identity and effective scope; Codex History performs no account-wide discovery; scope changes persist and take effect without a runtime reset. The generation receipt and Evidence manifest are retained as the only permitted input generation for BJ-10/AV-06.
...
- **Precondition:** current Evidence is the exact active generation produced by BJ-08 and contains at least one selected live leaf from Local Folder, GitHub, Feishu and Codex History. Those four leaves together provide at least two distinct top-level domains, a real subcategory in each domain, one cross-Connector relationship, tags, aliases, provenance, freshness and a known gap.
...
- **Oracle:** the immutable chain is `four ConnectorStatus receipts -> authorization/identity set -> scanPlanHash -> selectedLeafManifestHash -> active QMD generationReceiptHash -> evidenceManifestHash -> proposalHash/baseWikiHash -> Owner approval -> publishedVaultManifestHash`; every link resolves. `ConnectedSet = CommittedSet = ProposalSet = VaultSet = {local-folder, github, feishu, codex-history}`. The approved output has at least two useful primary folders and depth-two navigation; controlled hierarchical tags; one standard cross-Connector link visible in the graph report; a usable `index.md` per directory; explicit orphan/broken-link reports; valid OKF v0.2 and Obsidian profile; no second Agent/provider semantics. Rejection, stale hash, a substituted controlled-Local generation or any missing Connector provenance fails and leaves the Wiki byte-identical.
...
| EC-SOURCE-02 | Leave pagination/decision open, child count unknown, average Connector percentages, treat `0/0` as complete or omit known unenumerated slots. | Exact-set recomputation retains open/blocked state and affected discovery cannot reach 100%; any API/GUI mismatch fails. | Product-self-reported/averaged/rounded percentage, dropped cursor or guessed denominator. |
| EC-SOURCE-03 | Return Agent child outcomes with a missing/duplicate/extra/wrong target or repeated-parent action, then attempt an unselected leaf read before/after its ancestor descends without a valid target-bound `LeafSelectionReceipt`. | The malformed layer fails `AGENT_OUTPUT_INVALID` with no intent/receipt; both reads return `SCAN_BODY_READ_DENIED`; target and sibling read counters stay zero. | Best-effort subset, implicit skip, hidden orchestrator branch, parent descent treated as leaf selection, repeated parent enumeration or body-bearing evidence. |
...
| EC-WRITER-02 | Change any displayed diff, Evidence item or compiler receipt after review; substitute a Local-only/old generation for the four-live-Connector generation; or omit one Connector from proposal/Vault provenance. | Recomputed lineage/proposal hashes or exact Connector-set checks fail before mutation; active Vault remains byte-identical. | Four connected rows plus unrelated Wiki, provenance added after approval, mutable proposal or hash recalculation retaining old approval. |
...
- **Success standard:** BJ-02 through BJ-07 pass; every completed layer has a `childSetHash`, a decision-target set and an atomic child outcome set whose union with Control Plane system outcomes exactly covers the direct-child set; container descend creates an EnumerationIntent for the child and never re-lists the completed parent; leaf descend creates target-bound Decision/LeafSelection receipts before read. Layer Summary bodies stay in scratch and are deleted after decision while body-free hashes/outcomes/reasons remain inspectable. An independent oracle recomputes Discovery from complete-page nodes plus known unenumerated child slots, Summarization from summary-bearing versus intended layers, Selected Scan from processed versus selected leaves and Committed Index from active-manifest versus processed `qmd-current` leaves, globally and per Connector, then exactly matches API and GUI at every set change. Each Connector row exposes its own open/blocked state; only valid selections are read; narrowing freezes the excluded leaf's counter and removes its canary through a newly published QMD generation.
...
- **Preconditions:** the exact BJ-08 active QMD generation and Evidence manifest containing all four required live Connector types; existing approved Vault with unknown frontmatter; supported Obsidian installed.
...
- **Success standard:** BJ-10 and BJ-12 pass; the proposal and Vault resolve provenance for Local Folder, GitHub, Feishu and Codex History from the exact BJ-08 generation, and the full Connector-to-Vault Hash chain verifies. The Vault has at least two meaningful top-level folders, a real subfolder under each, controlled hierarchical tags, navigable `index.md` files and one explainable cross-Connector link; OKF/profile, CAS and exact proposal binding pass; `page_uid`, unknown keys and links survive; the Owner verifies Properties, tags, aliases, folders, backlinks and Graph without a required plugin; orphan and broken-link reports match the Vault.
...
- **Failure standard:** a substituted generation, missing live Connector provenance, broken Hash-chain link, flat or semantically useless taxonomy, missing directory index, unexplained graph relation, hidden semantic provider, approval bypass, byte change after reject/stale CAS, missing human checklist, `blocked` or `not-run` fails the gate.
...
| `CORE-AV-02` | Codex drives metadata-only root discovery, layer summaries, exact child outcomes, ask-user, pause and restart/resume while Sources renders the complete `All Sources + four Connectors` by `Discovery + Summarization + Selected Scan + Committed Index` 5x4 matrix. | Zero pre-selection body reads; every invocation binds the canonical `skillHash` and active policy-document hashes; a controlled priority/`WIKI.md` change produces the expected changed child outcome without expanding scope; every one of the 20 progress cells is independently recomputed from its own exact sets; an unknown denominator has no percentage; any displayed 100% is explicitly limited to the active Scan Plan frontier; one QMD failure recovers without duplicate logical credit. |
| `CORE-AV-03` | Four selected live leaves enter one active QMD generation and are used through product MCP, including explicit raw-exposure policy. | One generation manifest names all four Sources; current public query/get resolves four canaries; Visitor lists only `query`; Admin actions remain role/hash gated; explicit raw exposure returns only an approved normal canary and refuses sensitive/excluded canaries. |
...
| `CORE-AV-06` | One current Source item changes and the approved incremental path runs without reset. | Connector-specific metadata reconciliation records `recheckedMetadata` separately; only the changed branch repeats logical work; generation B excludes stale content; update proposal preserves unchanged `page_uid`; approved Concept and graph reflect the change. |
...
| `CORE-EC-02` | Request a repo/path/object/task outside approved scope before enumeration/body read; after commit, request raw exposure for one normal approved item, one sensitive-unapproved item and one excluded item. | Out-of-scope operations deny with zero access; explicit normal raw exposure follows policy; sensitive and excluded canaries never appear in response/diagnostics; provider counters, policy decision and redaction evidence are retained. |
...
| `CORE-EC-04` | Keep one cursor open or child count unknown while other Connectors advance, then attempt to start its Layer Summary and Agent decision. | The incomplete layer rejects summarization and Agent invocation with zero summary/decision/child-intent/body-read receipts; that Connector and All Sources show `completed / ?` with no percentage or completion; the independent 5x4 set report matches API and both viewports. |
...
- **Preconditions:** one candidate build, logged-in Codex CLI and Obsidian. Before the run, the Owner records a redacted acceptance input sheet with four exact authorized scopes; one current unique canary question/expected answer per Connector; one real three-level metadata path; one normal raw-exposure canary, one sensitive-unapproved canary and one excluded canary; one branch marked sensitive or constrained by a deliberately tight approved budget for `ask-user`; one authorized priority reference or narrowing `WIKI.md` rule with an expected child outcome and a counterfactual outcome when that rule is removed; one explicit known gap; two Source items that genuinely support one cross-Connector relation; and one changeable item/version for the incremental step. GitHub uses authenticated `gh`; Feishu uses the explicitly selected authenticated `lark-cli` profile; Codex uses explicit project/thread scope. The input sheet stores locators and expected facts by content hash or redacted label, never raw Source body.
- **Steps:** open the same Companion runtime at desktop and mobile widths; confirm all four Connector rows show real provider/version, redacted identity/profile, exact scope and state; open Agent and verify all six official rows, links, versions/states and disabled reasons; select Codex; approve one four-Source Scan Plan; start progressive discovery and verify only root direct-child metadata appears; follow the recorded three-level path, observing the current temporary Layer Summary before each decision commit and the durable `summaryHash`, outcomes and reasons afterwards; verify the recorded priority/`WIKI.md` rule is hash-bound to the invocation and produces its expected outcome, then remove it through an approved plan change and verify only the affected decision changes to the recorded counterfactual; trigger and resolve the recorded `ask-user`; pause, restart normally and resume; finish the four recorded leaves into the same active QMD generation while observing the complete All Sources plus four-Connector, four-dimension progress matrix; query the four canaries; explicitly request raw exposure for the recorded normal, sensitive and excluded items and verify only the approved normal item can be returned; complete one grounded writing, decision and retrospective request that exposes the recorded known gap; generate a proposal containing the recorded real cross-Connector relation plus meaningful top-level/sublevel folders, controlled tags, aliases, standard links, provenance and freshness; reject once and prove the Vault is byte-identical; regenerate, review the four-stage Connector lineage and approve; open the actual Wiki root in Obsidian; change the recorded item, run incremental scan, verify body-free metadata reconciliation and separately reported `recheckedMetadata`, approve the update proposal and reopen the updated Concept.
